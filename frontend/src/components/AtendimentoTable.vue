  ...
  <div class="table-responsive">
    <table class="table table-centered table-nowrap mb-0 rounded">
      <thead class="thead-light">
        <tr>
          <th class="border-0 rounded-start">Paciente</th>
      ...
      <tbody>
        ...
        <tr v-else-if="!atendimentos.length">
          <td colspan="5" class="text-center py-4">Nenhum atendimento encontrado</td>
        </tr>
        <tr v-for="atendimento in atendimentos" :key="atendimento.id">
          <td>{{ atendimento.paciente?.nome || '-' }}</td>
          <td>{{ formatarDataHora(atendimento.data_hora) }}</td>
          <td>{{ descricaoResumida(atendimento.descricao) }}</td>
          <td>
            <span class="badge" :class="atendimento.status === 'ativo' ? 'bg-success' : 'bg-secondary'">
              {{ atendimento.status === 'ativo' ? 'Ativo' : 'Inativo' }}
            </span>
          </td>
          <td class="text-center">
            <button class="btn btn-link text-dark p-1" @click="$emit('view', atendimento)" title="Visualizar">
              <svg class="icon icon-xs" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
            </button>
            <button class="btn btn-link text-dark p-1" @click="$emit('edit', atendimento)" title="Editar">
            ...
            <button class="btn btn-link text-danger p-1" @click="$emit('inactivate', atendimento)" title="Inativar" :disabled="atendimento.status === 'inativo'">
